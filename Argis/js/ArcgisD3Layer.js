/**
 * Created by 海东 on 2015/4/23.
 * 调用改模块，请在引入Arcgis js api前设置dojoConfig参数.
 * var dojoConfig = {
 *       paths: {lib: location.pathname.replace(/\/[^/]+$/, "") + "/js" }
 *       };
 */
define(
    [
	...
	], function (declare, lang, connect, map, esriRequest, GraphicsLayer, Point, webMercator) {
    	return declare([GraphicsLayer],{
			constructor: function(url, options) {
    		      			var self = this;
                            //增加下面这句话，会导致"calling chained constructor with inherited"错误
			...
			_bind: function(map){
				this._connects = [];
				this._connects.push( connect.connect( this._map, "onExtentChange", this, this._reset ) );
			},

			_project: function(x){
				var p = new Point( x[0], x[1] );
				var point = this._map.toScreen( webMercator.geographicToWebMercator( p ) );
			...
			_render: function(){
				...
				if ( this.type == 'circle' ) {
					...
					p.data( this.geojson.objects.counties )
